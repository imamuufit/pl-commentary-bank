...
const HPA_SITE_QUERY='site:h-power.sakura.ne.jp';
...
const buildHpaSearchUrl=query=>`https://www.google.com/search?q=${encodeURIComponent(`${HPA_SITE_QUERY} ${query} HPA 北海道パワーリフティング`)}`;
async function initFlexibleEventSearch(){const input=document.querySelector('#eventSearch');const hint=document.querySelector('#eventSearchHint');const openButton=document.querySelector('#openEventBtn');const hpaButton=document.querySelector('#openHpaSearchBtn');if(!input||!hint||!openButton)return;let eventConfig=null;try{const res=await fetch(`${EVENT_SEARCH_CONFIG_URL}?t=${Date.now()}`,{cache:'no-store'});eventConfig=await res.json();}catch(e){return;}const event=eventConfig.event||{};const render=()=>{const q=input.value.trim();if(!q)return;const score=scoreEvent(event,q);if(score<=0){hint.innerHTML='アプリ内の登録済み大会には一致しません。HPA検索で公式情報を確認してください。';return;}hint.innerHTML=`こちらの大会ですか？ <button type="button" id="eventSuggestionBtn">${event.name||'登録済み大会'}</button>`;document.querySelector('#eventSuggestionBtn')?.addEventListener('click',()=>{input.value=event.name||q;openButton.click();});};
...
hpaButton?.addEventListener('click',()=>{const query=input.value.trim()||event.name||'北海道 パワーリフティング 大会 結果';window.open(buildHpaSearchUrl(query),'_blank','noopener');});
render();}
